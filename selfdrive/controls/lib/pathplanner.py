...
def calc_states_after_delay(states, v_ego, steer_angle, curvature_factor, steer_ratio, delay):
    ...
    states[0].psi = v_ego * curvature_factor * \
        math.radians(steer_angle) / steer_ratio * delay
    return states
    ...
    def update(self, sm, pm, CP, VM):
        ...
        if (not active) or (self.lane_change_timer > LANE_CHANGE_TIME_MAX) or (not one_blinker) or (not self.lane_change_enabled):
            ...
            # starting
                ...
                self.lane_change_adjust_new = interp(
                    v_ego, self.lane_change_adjust_vel, self.lane_change_adjust)
                self.lane_change_ll_prob = max(
                    self.lane_change_ll_prob - self.lane_change_adjust_new*DT_MDL, 0.0)
                # 98% certainty
        ...
        if mode_select == 3:
            self.cur_state = calc_states_after_delay(
                self.cur_state, v_ego, angle_steers - angle_offset, curvature_factor, VM.sR, (CP.steerActuatorDelay + 0.05))
        else:
            self.cur_state = calc_states_after_delay(
                self.cur_state, v_ego, angle_steers - angle_offset, curvature_factor, VM.sR, CP.steerActuatorDelay)

        v_ego_mpc = max(v_ego, 5.0)  # avoid mpc roughness due to low speed
        self.libmpc.run_mpc(self.cur_state, self.mpc_solution,
                            list(self.LP.l_poly), list(
                                self.LP.r_poly), list(self.LP.d_poly),
        ...
        # str2 = 'delta2/{} fDES2/{}'.format(delta_steer2,
        ...
        if mpc_nans:
            ...
            self.cur_state[0].delta = math.radians(
                angle_steers - angle_offset) / VM.sR
        ...
        plan_send.valid = sm.all_alive_and_valid(
            service_list=['carState', 'controlsState', 'liveParameters', 'model'])
